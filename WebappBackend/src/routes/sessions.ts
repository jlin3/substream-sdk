...
import { notifyStreamStarted, notifyStreamEnded } from '../services/notifications';
 ...
 */
router.post('/end/:sessionId', authenticateToken, async (req: AuthRequest, res) => {
  try {
 ...
 */
router.get('/:sessionId', optionalAuth, async (req: AuthRequest, res) => {
  try {
    const { sessionId } = req.params;
 ...
 */
router.post('/:sessionId/viewers', optionalAuth, async (req: AuthRequest, res) => {
  try {
    const { sessionId } = req.params;
