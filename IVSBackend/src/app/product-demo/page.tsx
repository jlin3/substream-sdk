...
export default function ProductDemoPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-surface-50 text-white">
      {/* Nav */}
      <nav className="flex items-center justify-between px-6 py-4 border-b border-white/10 sticky top-0 bg-surface-50/90 backdrop-blur z-20">
      ...
      {/* Hero */}
      <header className="px-6 pt-20 pb-16 text-center border-b border-white/10">
        <div className="max-w-3xl mx-auto space-y-6">
          <p className="text-sm font-medium text-brand-400 uppercase tracking-widest">
            The Gaming Streaming SDK for Studios
          </p>
          <h1 className="text-4xl sm:text-6xl font-bold tracking-tight leading-tight">
            Turn any game into a
            <br />
            <span className="text-brand-400">live, monetizable platform.</span>
          </h1>
      ...
      <section className="px-6 py-20 border-b border-white/10">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-3">The full loop</h2>
          ...
          <div className="flex flex-wrap items-stretch justify-center gap-3 mb-14 text-sm">
            {[
              ...
              <div key={node} className="flex items-center gap-3">
                <span className="rounded-lg border border-white/10 bg-surface-100 px-4 py-2 font-medium">
                  {node}
                </span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-5">
            <Pillar
              num="1"
              title="Plugs into any game"
              body="5 lines of SDK code. Web, Unity, and iOS — any engine that renders frames."
              anchor="#integrate"
            />
            <Pillar
              num="2"
              title="Streams to your website"
              body="A viewer URL you can link or iframe. Sub-second latency, on your domain."
              anchor="#website"
            />
            <Pillar
              num="3"
              title="Data & content you own"
              body="Streams, recordings, viewers, and events — all in your dashboard and API."
              anchor="#data"
            />
            <Pillar
              num="4"
              title="Monetization options"
              body="Usage-based billing plus revenue models: subscriptions, watch parties, clips."
              anchor="#monetize"
            />
            <Pillar
              num="5"
              title="Highlights + custom models"
              body="AI clips out of the box, then fine-tuned to what's exciting in your game."
              anchor="#highlights"
            />
          </div>
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-surface-100 overflow-hidden">
          ...
          <div className="p-5">
            <p className="text-xs text-white/40 mb-3">
              Works with: {INTEGRATIONS[tab].engines}
            </p>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-start">
          <div>
            <p className="text-white/60 mb-6">
              Every stream returns a <code className="text-brand-300">viewerUrl</code> pointing at
                ...
                <tbody>
                  {PRICING.map((row) => (
                    <tr key={row.component} className="border-b border-white/5 last:border-0">
                      ...
                      <td className="px-4 py-3 font-mono text-brand-300 whitespace-nowrap">{row.rate}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div>
            ...
            <div className="space-y-4">
              {REVENUE_MODELS.map((m) => (
                <div key={m.title} className="rounded-xl border border-white/10 bg-surface-100 p-5">
      ...
      <Section id="highlights" eyebrow="Pillar 5" title="AI highlights — tuned to your game">
        ...
        <div className="rounded-xl border border-white/10 bg-surface-100 p-6 mb-8">
            ...
            {PIPELINE.map((p, i) => (
              <div key={p.step} className="space-y-2">
                <div className="flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-brand-600/20 border border-brand-500/30 text-brand-400 text-xs font-bold flex items-center justify-center">
                    {i + 1}
                  </span>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div className="rounded-xl border border-brand-500/30 bg-brand-600/10 p-6">
          ...
          <div className="rounded-xl border border-white/10 bg-surface-100 p-6">
            ...
            <Link href="/dashboard/highlights/compare" className="text-sm text-brand-300 hover:underline">
      ...
      {/* Run the demo */}
      <section id="run-the-demo" className="px-6 py-20 border-b border-white/10 bg-surface-100/40">
        <div className="max-w-5xl mx-auto">
          <p className="text-sm font-medium text-brand-400 uppercase tracking-widest text-center mb-2">
            Guided walkthrough
          </p>
          <h2 className="text-3xl font-bold text-center mb-3">Run the demo in 5 minutes</h2>
          ...
          <div className="space-y-4">
            {DEMO_STEPS.map((s) => (
              <div
                key={s.n}
                className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-xl border border-white/10 bg-surface-100 p-5"
              >
                <div className="w-10 h-10 shrink-0 rounded-full bg-brand-600/20 border border-brand-500/30 flex items-center justify-center text-brand-400 font-bold">
                  {s.n}
                </div>
                ...
                <Link
                  href={s.href}
                  className="shrink-0 rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium hover:bg-brand-500 transition-colors text-center"
                >
                  {s.cta}
                </Link>
              </div>
          ...
          {/* Demo credentials */}
          <div className="mt-10 rounded-xl border border-white/10 bg-surface-100 p-6">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-white/50 mb-4">
              Demo credentials
            </h3>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
              <Cred label="API" value={HOSTED_API} mono />
              <Cred label="Streamer ID" value="demo-child-001" mono />
              <Cred label="Publish token" value="demo-token" mono />
              <Cred label="Viewer token" value="demo-viewer-token" mono />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-3">
            {FAQS.map((f) => (
              <details
                key={f.q}
                className="group rounded-xl border border-white/10 bg-surface-100 p-5 [&_summary::-webkit-details-marker]:hidden"
              >
      ...
      <section className="px-6 py-20 text-center">
        <div className="max-w-2xl mx-auto space-y-6">
          <h2 className="text-3xl font-bold">Ready to see it on your game?</h2>
...
function Pillar({ num, title, body, anchor }: { num: string; title: string; body: string; anchor: string }) {
  return (
    <a
      href={anchor}
      className="block rounded-xl border border-white/10 bg-surface-100 p-5 hover:border-brand-500/40 hover:bg-surface-200 transition-colors"
    >
      <div className="w-8 h-8 rounded-full bg-brand-600/20 border border-brand-500/30 flex items-center justify-center text-brand-400 font-bold text-sm mb-3">
        {num}
      </div>
...
function Section({
  ...
  return (
    <section id={id} className="px-6 py-20 border-b border-white/10 scroll-mt-20">
      <div className="max-w-6xl mx-auto">
        <p className="text-sm font-medium text-brand-400 uppercase tracking-widest mb-2">{eyebrow}</p>
...
function InfoCard({ title, body }: { title: string; body: string }) {
  return (
    <div className="rounded-xl border border-white/10 bg-surface-100 p-5 space-y-2">
...
function Cred({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded-lg border border-white/10 bg-surface-50 px-4 py-3">
      <div className="text-xs text-white/40 mb-1">{label}</div>
      <div className={`text-sm break-all ${mono ? 'font-mono text-brand-300' : ''}`}>{value}</div>
    </div>
  );
}
