...
export function WatchPlayer({
  ...
  const initialPlaying = (): NowPlaying | null => {
  ...
  const [nowPlaying, setNowPlaying] = useState<NowPlaying | null>(initialPlaying);
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="relative bg-black">
            {nowPlaying ? (
              nowPlaying.type === 'live' ? (
                <div className="aspect-video flex items-center justify-center">
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-white/50 text-sm">Live viewer requires IVS Real-Time connection</p>
                    <Link href={`/dashboard/streams/${nowPlaying.id}`} className="text-brand-400 text-xs hover:underline">
              ...
              <div className="aspect-video flex items-center justify-center">
                <div className="text-center space-y-2">
                  <p className="text-white/30">No streams yet</p>
                  <Link href="/demo" className="text-brand-400 text-sm hover:underline">Start a stream from the SDK demo</Link>
                </div>
              </div>
            )}
            ...
            <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {nowPlaying.type === 'live' && (
                    <span className="text-xs px-1.5 py-0.5 rounded bg-live/20 text-live font-medium shrink-0">LIVE</span>
                  )}
                  {nowPlaying.type === 'highlight' && (
                    <span className="text-xs px-1.5 py-0.5 rounded bg-brand-600/20 text-brand-400 font-medium shrink-0">Highlight</span>
                  )}
                </div>
                <p className="text-xs text-white/40 mt-0.5">
                  {nowPlaying.streamer}
                  {nowPlaying.duration && <> &middot; {formatDuration(nowPlaying.duration)}</>}
                </p>
              </div>
              {nowPlaying.type === 'highlight' && (
                <Link href={`/dashboard/highlights/${nowPlaying.id}`} className="text-xs text-brand-400 hover:underline shrink-0">
                  View Analysis
                </Link>
              )}
            </div>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-hide">
                {completedHighlights.map((h) => (
                  <button
                    ...
                    className={`shrink-0 w-44 rounded-lg overflow-hidden border transition-colors text-left ${
                      nowPlaying?.id === h.id ? 'border-brand-500 bg-brand-600/10' : 'border-white/10 bg-surface-200 hover:border-white/20'
                    }`}
                  >
          ...
          <div className="px-4 py-4 flex-1 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                {recordings.map((r) => (
                  <button
                    ...
                    className={`rounded-lg overflow-hidden border transition-colors text-left ${
                      nowPlaying?.id === r.id ? 'border-brand-500 bg-brand-600/10' : 'border-white/10 bg-surface-200 hover:border-white/20'
                    }`}
                  >
                    <div className="aspect-video bg-surface-300 flex items-center justify-center relative">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-white/10">
            ...
            {liveStreams.length > 0 && (
              <div className="mt-6">
                <h3 className="text-xs font-semibold uppercase tracking-wide text-live mb-3 flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                  {liveStreams.map((s) => (
                    <button
                      ...
                      className={`rounded-lg overflow-hidden border transition-colors text-left ${
                        nowPlaying?.id === s.id ? 'border-live bg-live/10' : 'border-white/10 bg-surface-200 hover:border-live/50'
                      }`}
                    >
                      <div className="aspect-video bg-surface-300 flex items-center justify-center">
                        <span className="text-live text-xs font-semibold">● LIVE</span>
                      </div>
                      <div className="p-2">
                        <p className="text-xs font-medium truncate">{s.title || 'Live Stream'}</p>
                        <p className="text-[10px] text-white/30 mt-0.5">{s.streamerName || s.streamerId}</p>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="flex items-center justify-between gap-1">
              {REACTIONS.map((r) => (
                <button
                  ...
                  className="flex-1 py-2 rounded-lg bg-surface-200 hover:bg-surface-300 transition-colors text-lg active:scale-110"
                  title={r.label}
                >
                  {r.emoji}
                </button>
          ...
          <form onSubmit={addComment} className="px-4 py-3 border-t border-white/10">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-surface-200 border border-white/10 rounded-lg px-3 py-2 text-xs placeholder:text-white/20 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
              />
              <button
                type="submit"
                disabled={!commentInput.trim()}
                className="rounded-lg bg-brand-600 px-3 py-2 text-xs font-medium hover:bg-brand-500 transition-colors disabled:opacity-30"
              >
                Send
              </button>
            </div>
